...
/**
 * Completes a session Supabase-js already established client-side — a Google
 * (or any future OAuth provider) sign-in, or a password-recovery link
 * exchange. Both are PKCE code exchanges that have to happen client-side (the
 * code_verifier Supabase-js stashed lives in that same browser), so by the
 * time this runs the client already holds a real Supabase session — this
 * function's only job is verifying that session is genuine before trusting
 * it, never accepting the tokens at face value just because a client sent
 * them (same "derive, don't trust" principle every other real identity input
 * in this codebase follows). Real verification: re-checking the access token
 * against Supabase itself (getUserForToken), not just decoding the JWT
 * locally.
 */
export async function verifyExternalSession(accessToken: string): Promise<{ id: string; email: string }> {
