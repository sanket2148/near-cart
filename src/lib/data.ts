...
export type Product = {
  ...
  category: string;
  inStock: boolean;
  /** Present only when the shop tracks quantity; absent means "don't show a stock count." */
  stockQty?: number;
...
export type Shop = {
  ...
  etaMinutes: number;
  isOpen: boolean;
  /** Real, hours-aware human label (e.g. "Open · closes 9:00 PM") when set; absent for older callers. */
  openLabel?: string;
  deliveryFee: number;
  freeAbove: number;
  area: string;
